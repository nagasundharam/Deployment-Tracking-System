            ...
            steps {
                script {
                    ...
                    try {
                        ...
                        // Extract the ID using sed (universally available on Linux agents)
                        writeFile file: 'response.json', text: response
                        env.DEPLOYMENT_ID = sh(script: "sed -n 's/.*\"_id\":\"\\([a-f0-9]*\\)\".*/\\1/p' response.json | tail -1", returnStdout: true).trim()
